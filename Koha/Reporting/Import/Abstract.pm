...
sub itemItemTypeOkm{
    ...
    if(defined $data->{itemtype}){
        ...
        if($type eq 'KI' || $type eq 'AR'){
            $result = 'Kirjat';
        }
        elsif($type eq 'NU'){
            $result = 'Nuotit ja partituurit';
        }
        elsif( $cnPrimary && $cnPrimary eq '78' && ($type eq 'KA' || $type eq 'CD' || $type eq 'LP' || $type eq 'MP' || $type eq 'SI') ){
            $result = 'Musiikkiäänitteet';
        }
        elsif( $cnPrimary && $cnPrimary ne '78' && ($type eq 'KA' || $type eq 'CD' || $type eq 'LP' || $type eq 'MP' || $type eq 'SI') ){
        ...
        elsif($type eq 'LA' || $type eq 'SL' || $type eq 'DI' || $type eq 'ES' || $type eq 'KR' || $type eq 'KP' || $type eq 'MF' || $type eq 'MK' || $type eq 'MV' || $type eq 'PE' || $type eq 'TY'){
            $result = 'Muut aineistot';
        }
        elsif($type eq 'BR' || $type eq 'DV'){
...
sub itemIsYle{
    ...
    if($dimension){
        ...
        if($marc){
            ...
            if(defined $value){
               $_ = $value;
               if( /YLE myynti/ || /YLE Tallennemyynti/){
                  $result = 1;
               }
            }
        }
...
sub initLocationTypeData{
    ...
    my $adultLocations = {'AIK'=>1, 'VAR'=>1, 'MUS'=>1, 'LEH'=>1, 'KOT'=>1, 'PALVELUT'=>1, 'LUKIO'=>1, 'K'=>1, 'OUV'=>1, 'OU'=>'1', 'VAREILAI' => 1};
    my $childLocations = {'LN'=>1, 'KOULU'=>1, 'VARLN'=>1};
