...
sub public_itemtype {
    my ($params) = @_;
    my $item = $params->[0]->{item};
    my $it = C4::ItemType->get($item->{itype});
...
sub public_oplibLabel {
    ...
    my $locationCode = C4::OPLIB::Labels::getLabel( $item->{homebranch}, $item->{permanent_location}, $item->{itype}, $item->{ccode} )  if $item->{permanent_location};
    $locationCode = C4::OPLIB::Labels::getLabel( $item->{homebranch}, $item->{location}, $item->{itype}, $item->{ccode} ) if !$item->{permanent_location};
    return $locationCode;
}
