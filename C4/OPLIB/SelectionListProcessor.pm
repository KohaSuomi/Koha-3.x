package C4::OPLIB::SelectionListProcessor;
...
@RETURNS Arrayref of C4::OPLIB::SelectionList-objects
=cut
...
sub splitToSubSelectionLists {
    ...
    foreach my $record (@$records) {
        my $newname = ($params->{singleList}) ? $params->{singleList} : C4::OPLIB::SelectionList::GetSelectionListIdentifier($record);
        $recordsGroupedBySelectionList{$newname} = C4::OPLIB::SelectionList->new($record, $params->{singleList}) unless $recordsGroupedBySelectionList{$newname};
...
sub _loadFile {
    ...
    if ($params->{format} =~ /ISO2709/i) {
        $records = C4::ImportBatch::RecordsFromISO2709File($params->{file}, 'biblio', $marc_encoding);
    }
    elsif ($params->{format} =~ /MARCXML/i) {
        $records = C4::ImportBatch::RecordsFromMARCXMLFile($params->{file}, $marc_encoding);
    }
