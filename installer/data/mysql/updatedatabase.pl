...
if ( CheckVersion($DBversion) ) {
    ...
    #but this approach has better long-term maintainability outside of Koha master. (less coupling)
    print "Upgrade to $DBversion done (Bug XXX - Delayed Overdue stuff)\n";
    SetVersion ($DBversion);
}
...
if(CheckVersion($DBversion)) {
    ...
    print "Upgrade to $DBversion done (Bug XXX: Add biblio delete (delete_catalogue) permission.)\n";
    SetVersion($DBversion);
}
...
if(CheckVersion($DBversion)) {
    $dbh->do(q{
        CREATE TABLE `biblio_data_elements` ( -- stores extracted MARC key indicators for easy access.
...
if ( CheckVersion($DBversion) ) {
    ...
    print "Upgrade to $DBversion done (Bug XXX - Add Overdues Calendar)\n";
    SetVersion ($DBversion);
}
...
if (CheckVersion($DBversion)) {
    ...
    print "Upgrade for batchOverlayRules complete.\n";
    SetVersion($DBversion);
}
